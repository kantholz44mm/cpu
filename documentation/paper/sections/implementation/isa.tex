...
\subsubsection{Memory Model} \label{sec:implementation_isa_memory_model}
...
Each of these operations must be assigned its unique opcode so the control logic knows what operation to perform during a cycle. As discussed in \autoref{sec:implementation_isa_memory_model}, an operation can be emulated by using another operation. Therefore, the number of actual opcodes defined in an \ac{isa} may be smaller than the number of usable instructions within a programming language. However, there is no general way to substitute the fundamental arithmetic operations of addition, subtraction and bitwise shifting to the left or right. Therefore, these four operations receive one opcode each. The bitwise logic operations \texttt{AND}, \texttt{OR}, \texttt{NOT} can be represented by several other combinations of logic operations, as discussed in \autoref{sec:logical_operators}. Since the combination of these three operators is functionally complete, any set of operators that are used to substitute them must also be functionally complete.
...
While the \ac{isa} could simply define these three operations directly, that would result in a total of 7 operations the \ac{alu} can perform. This is suboptimal, since it would require $\lceil log_2(8) \rceil = 3$ bits to encode, wasting the eighth value representable by these 3 bits. Therefore, the operator set \texttt{\{OR, AND, XOR, NAND\}} is chosen. Both the \texttt{NAND} and \texttt{XOR} operators can substitute the \texttt{NOT} operator by using an operand and the same operand or the constant 1, respectively. This combination of operators is chosen because when built from only \texttt{NAND} gates, the logical expressions share large amounts of boolean expressions. Therefore, much of the intermediary results can be used for several of the operators.
