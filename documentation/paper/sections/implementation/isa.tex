...
A set of four functionally complete logical operations is chosen in order to fully exhaust the space indexed by 3 bits. Specifically, the operator set \texttt{\{OR, AND, XOR, NAND\}} is chosen. Both the \texttt{NAND} and \texttt{XOR} operators can substitute the \texttt{NOT} operator by using an operand and the same operand or the constant 1, respectively. This set of operators is chosen because when built from only \texttt{NAND} gates, the logical expressions share large amounts of boolean subexpressions . Therefore, much of the intermediary results can be used for several of the operators.
