...
Since all useful electrical circuits use passive components such as capacitors, resistors and inductors of multiple values and sizes, these will not be explored in detail here. All of the passive components use a 0402 package, as it is the smallest and cheapest package available for automated assembly in most consumer-grade assembly houses. The nominal voltage of \SI{5}{\volt} is chosen for all electrical designs, since it is the voltage used by many logic components, microcontrollers and memory \acp{ic}. Likewise, the \ac{usb} bus used for communicating with the \ac{cpu} uses \SI{5}{\volt} as well.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{images/SN74HC00_pinout.png}
    \caption[Pin mapping of SN74HC00 IC]{Functional pinout and internal gate layout of the SN74HC00 IC (SNx4HC00 datasheet, Rev. H \cite[p. 1]{ti_sn74hc00})}
...
\subsubsection*{Human Machine Interface} \label{sec:hardware_hmi}

\paragraph*{Mechanical input}

To interface with the user, mechanical inputs are needed. To enable user input, simple tactile switches are used. They are cheap and readily available. However, like all mechanical switches, they suffer from signal bouncing. In such mechanical switches, some physical contacts have to approach each other at finite speeds. Therefore, the imperfect surfaces of the contacts create arcing and do not touch instantaneously. This results in an oscillation of the output signal until it settles into the new state. The output at position \texttt{OND} in \autoref{fig:hardware_components_switch_debounce} shows the direct output of the mechanical switch without any debouncing circuitry.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{images/drawio/switch_debounce.png}  % Replace with your image file
    \caption[Switch debouncing]{Mechanical switch circuit and the graphs for mechanical position and output signal at relevant net sections (Modified from Clive Maxfield of Digikey Electronics, omitted description of bounces and outline of switch \cite{maxfield_hardware_debounce})}
