...
\begin{figure}[h!]
    ...
    \caption{Critical path shown in red, assuming a propagation delay of \SI{20}{\nano\second} per AND-Gate and \SI{5}{\nano\second} per NOT-Gate}
...
Within such a latch, there are clearly cycles, where the output of each NAND gate serves as the input of the other NAND gate. The boolean expressions for $Q$ and $Q'$ are as follows:
...
Even though one can write an expression for each path through the latch, when expanding any one of the two, there is an unbounded recursion:
