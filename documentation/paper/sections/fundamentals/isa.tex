...
An \ac{isa} is an abstract model and does not specify \textit{how} it is to be implemented. When a \ac{cpu} implementing a specific \ac{isa} is designed, the designers are largely free to choose the exact implementation details, as long as the end product adheres to the rules laid out by the \ac{isa}. The distinction between an \ac{isa} and an implementation of said \ac{isa} is important; an \ac{isa} by itself cannot run code. There may be multiple implementations of one \ac{isa} using vastly different electrical or logical technologies. Due to the boolean logic transformation rules mentioned in \label{sec:logical_operators}, there is an infinite number of possible implementations, even when the same electrical components are used. Many \ac{isa} also leave room for interpretation or choice when it comes to electrotechnical details such as the voltage levels used, etc.
