...
For this project, minimizing the number of gates required is the core metric. Therefore, the addressing modes that need additional arithmetic operations are not implemented. This, in turn, means that accessing sequential members of an array requires an additional operation to advance the index via addition, lowering throughput. The following modes are included in the \ac{isa}:

\begin{itemize}
    \item Register
    \item Immediate
    \item Register Indirect
    \item Immediate Indirect
\end{itemize}

These modes can be differentiated nicely by using two control lines, one each for deciding the source (register or immediate) and directness.

\subsection{Instruction Set}

The instruction set specifies what the \ac{cpu} can do. It lists all available operations and details all operands, affected flags/registers and the effects of the operation when executed. The effects of an instruction can range in complexity. This chapter explains the choices made when designing the \ac{isa} implemented in this project.
