...
The software simulation is intended to design, test and simulate a possible implementation of the \ac{isa} on the level of individual logic gates. It allows an estimation of the size, complexity and number of components of its hardware equivalent. The hardware will eventually consist of individual, discrete logic gates. Therefore, the simulation is used to verify a design on this level of abstraction. It also offers low cost and high speed of iteration. If a graphical tool is used, it also provides an opportunity visualize the entire structure. For this work, the digital logic simulator ``Digital'' by Heinz Neemann is used \cite{neemann_digital}. It is free to use and well documented and was previously used by the author to simulate other logical circuits. For these reasons, the cost of entry in both money and time spent learning to use the tool are minimal. The tool simulates digital logic circuits, which can then be hierarchically combined to form more and more complex circuits. \autoref{fig:digital_full_adder} shows a circuit that implements a 1 bit full adder within the software. \autoref{fig:digital_full_adder_reuse} shows how this circuit can then be reused in higher-level circuits.
...
\begin{figure}[H]
    ...
    \caption[Hierarchical reuse of logic gate circuits in ``Digital'']{Hierarchical reuse of the 1 bit full adder shown in \autoref{fig:digital_full_adder}}
...
\begin{figure}[H]
    ...
    \caption[Gated, clocked D latch]{Two sequential D latches, forming a gated, clocked D latch}
...
\begin{itemize}
    \item \textbf{Reset} Clears all registers
    \item \textbf{Halt} Manually halts the device, stopping clock propagation
    \item \textbf{Continue} Manually resumes operation if the device is halted
    \item \textbf{Step} Manually execute one phase of the instruction cycle
    \item \textbf{Clken} Enable or disable clock signal generation     
\end{itemize}
...
\subsubsection{Analysis} \label{sec:simulation_analysis}

The full simulation contains 2041 individual 2-input \texttt{NAND} gates. While the entire simulation is too large to show here, an overview can be seen in \appref{app:simulation_full}. \autoref{fig:implementation_simulation_gate_pie_chart} shows how the gates are distributed among the individual logical modules. From this graphic, it becomes apparent that a large portion of all \texttt{NAND} gates are used to construct registers. In total, $\frac{9 \cdot 92}{2041} \approx 40.6\%$ are used for this purpose. When considering the \ac{alu}, only $\approx 21.6\%$ of all gates do actual computation. 
\\

While the simulation can run as fast as the host computer can process it at, a physical implementation succumbs to propagation delays and is therefore limited in its operating frequency. The device contains 31 non-recursive subtrees, of which the longest one contains 84 \texttt{NAND} gates. This means that the theoretical maximum clock frequency of the device, depending on the propagation delay of a single \texttt{NAND} gates, is given by

\begin{displaymath}
    \mathit{f}_{max}(t_{NAND}) = \frac{1}{t_{NAND} \cdot 84}
\end{displaymath}

For example, at a propagation delay of \SI{10}{\nano\second}, this results in a maximum operating frequency of $\approx\SI{11.9}{\mega\hertz}$. However, this is a rough estimation and does not consider factors such as the propagation delay added by electrical connections and non-gate components.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/gate_pie_chart.png}
    \caption[Distribution of NAND gates among modules]{Distribution of NAND gates among logical modules, given in absolute number and percentages relative to total}
    \label{fig:implementation_simulation_gate_pie_chart}
\end{figure}
