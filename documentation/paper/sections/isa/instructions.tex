\subsection{Opcodes} \label{sec:implementation_opcodes}
...
\begin{figure}[H]
    ...
    \caption[OR, AND, XOR and NAND built from only NAND gates]{Logic gates implementing OR, AND, XOR, NAND while reusing intermediary results}
...
\end{align}
\captionof{table}[OR, AND, XOR, NAND using only the NAND operator]{Boolean expressions for binary operators OR, AND, XOR, NAND using only NAND operators}
\label{align:binary_operators_from_nand}
\vspace{\baselineskip}

However, this only defines the set of operations the \ac{alu} can perform by itself. It does not define the environment under which the \ac{alu} in invoked. For example, an arithmetic instruction may use operands from different data sources such as registers, an immediate value encoded in the instruction or memory. From past experience, it also proves useful to have arithmetic instructions that do not modify the state of the flags register. An example of a use case for this is when implementing a stack in software. When pushing a value to this stack, the stack pointer must be incremented or decremented. If no addition can be performed without modifying the flags register, executing a \texttt{PUSH} instruction would therefore modify it as well. Therefore, early revisions of the \ac{ava} \ac{isa} defined a ``flags'' bit as part of the opcode that specified whether the flag output of the \ac{alu} shall update the flags register ``R5'' or not. 
\\
...
\begin{table}[H]
    ...
    \end{tabular}    
    \caption[Instruction suffixes]{Effect of instruction suffix on arithmetic instructions (XXX represents an arbitrary arithmetic operation)}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|l|} 
        ...
        \hline
    \end{tabular}
    \caption[AVA instruction set]{Supported instructions and their opcodes, variants, type and brief description (variant suffices are explained in \autoref{tab:instruction_suffices})}
